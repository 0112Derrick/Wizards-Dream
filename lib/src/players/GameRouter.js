...
(function (ClientMapSlot) {
    ...
    ClientMapSlot[ClientMapSlot["ClientSkillTree"] = 4] = "ClientSkillTree";
...
export class GameRouter {
        ...
        if (this.clientMap.has(_data.id)) {
            switch (slot) {
                ...
                case ClientMapSlot.ClientSkillTree:
                    this.clientMap.get(_data.id)?.setSkillTree(_data.arg);
                    break;
            }
        }
    }
    ...
    initGame(_socket, _ip) {
        ...
        _socket.on($socketRoutes.REQUEST_CLIENT_ACTION_MESSAGE, (message) => {
            this.addCharacterActionRequestToQueue(message);
        });
        ...
        _socket.on($socketRoutes.REQUEST_ADD_CREATED_CHARACTER, (character, map, clientID) => {
            ...
            if (char) {
                gameRouter.clientMap.has(clientID)
                    ? gameRouter.setClientMap({ id: clientID, arg: char }, ClientMapSlot.ClientActiveCharacter)
    ...
    serverTick() {
        setInterval(() => {
            ...
            while (!this.clientMessageQueue.isEmpty()) {
                ...
                const { action, worldWidth, worldHeight, mapMinWidth, mapMinHeight } = message.action;
                if (!message.action) {
                    break;
                }
    ...
    checkIfCharacterCanUseSkill(skill, id) {
        ...
        if (foundSkill.Dependencies.class != null &&
    ...
    playerJoinServer(playerID, server) {
        ...
        try {
            ...
            gameRouter.io
                .to(socket.id)
                .emit($socketRoutes.RESPONSE_SERVER_MESSAGE, `Welcome to ${server}`, "Server");
        }
        catch (error) {
