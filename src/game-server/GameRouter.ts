import {
  ServerSizeConstants as $serverSize,
  SocketConstants as $socketRoutes,
...
import {
  ...
  CharacterData_Direction,
} from "./interfaces/CharacterInterfaces.js";
import $Queue from ".././framework/Queue.js";
import { MapNames } from "../constants/MapNames.js";
import { Overworld_Server } from "./Overworld_Server.js";
import {
  OverWorld_MapI as $OverWorld_MapI,
  syncOverworldTransmit as $syncOverworld,
} from "./interfaces/OverworldInterfaces.js";
...
export class GameRouter {
    ...
    if (this.clientMap.has(_data.id)) {
      switch (slot) {
        ...
        case ClientMapSlot.ClientActiveCharacter:
          this.clientMap
            .get(_data.id)
            ?.setActiveCharacter(_data.arg as $Character);
        ...
        case ClientMapSlot.ClientInputQue:
          this.clientMap
            .get(_data.id)
            ?.addInput(_data.arg.tick, _data.arg.input);
          break;
  ...
  deleteFromActiveSocket(socket: string): boolean {
    for (
      let i = 0;
      i < GameRouter.GameRouterInstance.activeSockets.length;
      i++
    ) {
  ...
  initGame(_socket: Socket, _ip: string) {
    ...
    _socket.on(
      ...
      (clientID: string, serverRoom: string) =>
        this.playerJoinServer(clientID, serverRoom)
    );
    _socket.on($socketRoutes.REQUEST_CLIENT_LOGOUT, this.playerLogout);
    _socket.on($socketRoutes.REQUEST_MESSAGE, this.checkMessage);
    _socket.on(
      $socketRoutes.REQUEST_OVERWORLD_GAME_OBJECTS,
      this.updateGameObjects
    );
    _socket.on($socketRoutes.REQUEST_PING, (id) => {
      console.log("request from:", id);
      GameRouter.GameRouterInstance.io.to(id).emit($socketRoutes.RESPONSE_PONG);
    });
    _socket.on($socketRoutes.REQUEST_CURRENT_TICK, (id) => {
      console.log("request from:", id);
      GameRouter.GameRouterInstance.io
        .to(id)
        .emit($socketRoutes.RESPONSE_CURRENT_TICK, this.currentServerTick);
      console.log(this.currentServerTick);
    });
    _socket.on(
      $socketRoutes.REQUEST_CLIENT_ACTION_MESSAGE,
      (message: $MessageHeader) => {
        this.addCharacterActionRequestToQueue(
          message
    ...
    _socket.on(
      $socketRoutes.REQUEST_CHARACTER_MOVEMENT,
      gameRouter.addCharacterMoveRequestsToQueue
    );
    _socket.on(
      $socketRoutes.REQUEST_ADD_CREATED_CHARACTER,
      (character: any, map: MapNames, clientID: string) => {
        let implementsInterface =
          utilFunctions.checkIfObjectMeetsCharacterDataInterface(character); //this.implementsCharacterDataInterface(character)
        ...
        if (char) {
          gameRouter.clientMap.has(clientID)
            ? gameRouter.setClientMap(
                { id: clientID, arg: char },
                ClientMapSlot.ClientActiveCharacter
              )
          ...
          console.log(
            ...
            clientID,
            " active character: ",
            gameRouter.clientMap.get(clientID).getActiveCharacter().username
          );
        }
  ...
  createMessageHeadersForActiveClients(): Array<{
    id: string;
    messageHeader: $MessageHeader;
  }> {
    let serverMessageHeaders: Array<{
      id: string;
      messageHeader: $MessageHeader;
    }> = [];
    ...
    if (!this.clientMessageQueue.isEmpty()) {
      ...
      que.forEach((client) => {
        ...
        let foundClientIterationNumb = this.getClientMap()
          .get(client.id)
          .getAdjustmentIteration();
        if (!foundClient) {
    ...
    for (let client of this.getClientMap().values()) {
      ...
      if (tickAdjustments.has(id)) {
        let messageHeader = this.createMessageHeader(
          id,
          client.getAdjustmentIteration(),
          null,
          tickAdjustments.get(id)
        );
  ...
  serverTick() {
    ...
    setInterval(() => {
      ...
      while (!this.clientMessageQueue.isEmpty()) {
        ...
        const { action, worldWidth, worldHeight, mapMinWidth, mapMinHeight } =
          message.action as $MessageContentsI;
        ...
        if (message.type == $serverMessages.Movement) {
          ...
          let coords = $MovementSystem.updateCharacterPosition(
            clientCharacter,
            action,
            worldWidth,
            worldHeight,
          ...
          let skill = message.action.action;
      ...
      if (serverMessageHeaders)
        serverMessageHeaders.forEach(
          (messageHeader: { id: string; messageHeader: $MessageHeader }) => {
            let id: string = null;
            ...
            GameRouter.GameRouterInstance.io
              .to(id)
              .emit($socketRoutes.RESPONSE_CLIENT_ACTION_MESSAGE, header);
          }
        );
  ...
  checkClientMessagesForIncorrectTickTiming(message: $MessageHeader) {
    ...
    switch (this.clientsTickChecker(clientTick)) {
      case 1:
        console.log(
          "Message was dropped by the server. Tick was behind schedule. " +
            "\n client tick:" +
            clientTick +
            " server tick: " +
            this.currentServerTick
        );
        this.adjustClientsTick(
          this.currentServerTick + clientTickAdjustment - clientTick,
          clientId
        );
        break;
      ...
      case 2:
        console.log(
          "Client is too far ahead of the server." +
            "\n client tick:" +
            clientTick +
            " server tick: " +
            this.currentServerTick
  ...
  adjustClientsTick(tickAdjustmentAmount: number, id: string) {
    ...
    client.setAdjustedTick(
      client.getAdjustmentIteration(),
      tickAdjustmentAmount
    );
  }
  ...
  sendServersList() {
    let gameRouter = GameRouter.GameRouterInstance;
    gameRouter.io.emit($socketRoutes.RESPONSE_ACTIVE_SERVERS, [
      ...gameRouter.serverRooms,
    ]);
  }
  ...
  checkIfCharacterCanUseSkill(skill: $SkillI, id: string): boolean {
    ...
    if (
      foundSkill.Dependencies.class != null &&
      foundSkill.Dependencies.class != clientCharacter.class
    ) {
      return false;
    }

    if (
      foundSkill.Dependencies.costCategory.length == 0 ||
      !foundSkill.Dependencies.costCategory
    ) {
      return true;
    }
    ...
    for (let costCategory of foundSkill.Dependencies.costCategory) {
      if (costCategory.toLowerCase() == "sp") {
        if (
          clientCharacter.attributes.sp >= foundSkill.Dependencies.costAmount
        ) {
          return true;
        }
      }
      ...
      if (costCategory.toLowerCase() == "hp") {
        if (
          clientCharacter.attributes.hp > foundSkill.Dependencies.costAmount
        ) {
          return true;
        }
      }
    ...
    let syncedOverworld: $syncOverworld = {
      grassyfield: {
        ...
        activePlayers: Object.fromEntries(
          overworld.grassyfield.activePlayers.entries()
        ),
      ...
      hallway: {
        ...
        activePlayers: Object.fromEntries(
          overworld.hallway.activePlayers.entries()
        ),
  ...
  syncPlayersMovements(charactersMovementData: Array<CharacterMovementData>) {
    GameRouter.GameRouterInstance.io.emit(
      $socketRoutes.RESPONSE_SYNC_PLAYERS_MOVEMENTS,
      charactersMovementData
    );
  }
  ...
  disconnectPlayer(id: any) {
    ...
    let foundCharacter = utilFunctions.findObjectByNameInArray(
      character.name,
      map.gameObjects
    );
    if (!foundCharacter) {
    ...
    let arr = utilFunctions.removeObjectFromArray(
      map.gameObjects,
      "name",
      foundCharacter.name
    );
    ...
    if (delCharacter) {
      console.log(
        `${foundCharacter.name} is no longer active: ${delCharacter}`
      );
    }
  ...
  getOverworld() {
    ...
    if (
      GameRouter.GameRouterInstance.Overworld == null ||
      GameRouter.GameRouterInstance.Overworld == undefined
    ) {
      let overworld = (GameRouter.GameRouterInstance.Overworld =
        new Overworld_Server());
      return overworld;
    } else {
  ...
  updateGameObjects(map: MapNames = MapNames.GrassyField) {
    ...
    switch (map) {
      case MapNames.GrassyField:
        gameObjects =
          GameRouter.GameRouterInstance.getOverworld().grassyfield.gameObjects;
        break;

      case MapNames.Hallway:
        gameObjects =
          GameRouter.GameRouterInstance.getOverworld().hallway.gameObjects;
        break;
  ...
  checkMessage(serverRoom: string, message: string, user): void {
    ...
    gameRouter.io
      .to(serverRoom)
      .emit($socketRoutes.RESPONSE_MESSAGE, cleanMessage, user);
  ...
  addCharacterToOverworld(
    ...
    if (
      !overworld.maps.get(overworldMap).activePlayers.has(character.username)
    ) {
      overworld.maps
        .get(overworldMap)
        .activePlayers.set(character.username, character);
    } else {
      gameRouter.syncOverworld();
      return;
    }
    ...
    overworld.maps.get(overworldMap).gameObjects.forEach((gameObj, i) => {
      if (utilFunctions.checkIfObjectMeetsCharacterDataInterface(gameObj)) {
        if (
          (gameObj as $characterDataInterface).username == character.username
        ) {
          console.log(
            `${character.username} already exists in ${
              overworld.maps.get(overworldMap).name
            } map gameObjects list.`
          );
          return;
        }
      }
  ...
  addCharacterMoveRequestsToQueue(
    characterMovingDirection: $Direction,
