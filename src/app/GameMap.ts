...
import {
  ClientController as $ClientController,
  ClientController,
...
export class GameMap implements MapI, gameMapGameObjectsI {
  ...
  draw() {
    ...
    const imageWidth = Math.min(
      this.camera.width,
      this.lowerImage.width - backgroundX
    );
    const imageHeight = Math.min(
      this.camera.height,
      this.lowerImage.height - backgroundY
    );
  ...
  handleCollisions(skill: $Skill) {
    this.gameObjects
      .filter((characterObj) => characterObj instanceof Character)
      .forEach((characterObj) => {
        if (skill.Shape.type == ShapeTypes.Rectangle) {
          if (
            this.intersector.isRect2DColliding(
              {
          ...
          if (
            this.intersector.isRectCollidingWithCircle(
              {
                width: (characterObj as Character).width,
                height: (characterObj as Character).height,
                x: (characterObj as Character).x,
  ...
  updateCharacter(character: Character) {
    ...
    if (currentDirection) {
      ...
      let pos = this.movementSystem.updateCharacterPosition(
        character,
        currentDirection,
        this.worldWidth,
        this.worldHeight,
  ...
  get ActiveCharacters() {
    if (!this.activeCharacters) {
      return (this.activeCharacters = new Map<
        string,
  ...
  addCharacter(character: Character): boolean {
    ...
    console.log(
      "Player:" + character.name + " has been added to: " + this.name
    );
    return true;
  }
  ...
  removeCharactersFromGameObjectsList(
    newCharacters: Map<string, $characterDataInterface>,
    map: GameMap
  ) {
  ...
  syncGameObjects(playersList: Array<GameObject>): void {
    console.log(
      "Synced game objects: " + playersList + "\nType: " + typeof playersList
    );
  ...
  syncActiveCharacters(
    activeCharactersList: Map<string, $characterDataInterface>
  ): void {
