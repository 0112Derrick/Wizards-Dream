...
export class Skill extends $GameObject implements $SkillI {
  ...
  get Dependencies(): skillDependency {
  ...
  set GameObjectsCallback(
    object: $characterAddAndRemoveGameObjectsFromRenderI
  ) {
  ...
  private removeSelfFromRenderedGameObjectsListCallback() {
    let result =
      this.gameObjectsListCommandsCallback.removeSkillFromRenderContex(this);
    if (result) {
      return;
    }
  ...
  draw(
    ...
    if (this.sprite) {
      if (this.shape.type == ShapeTypes.Rectangle) {
        if (
          !camera.isInsideOfView(
            (this.shape as Rectangle).x,
            (this.shape as Rectangle).y,
            (this.shape as Rectangle).width,
            (this.shape as Rectangle).height
          )
        ) {
          return;
        }
      ...
      setTimeout(() => {
        this.removeSelfFromRenderedGameObjectsListCallback();
        clearInterval(animationFrame);
      }, this.effectTime * this.cycle);
      return;
    }
    ...
    if (this.shape.type == ShapeTypes.Circle) {
      ...
      if (
        ...
        ctx.arc(
          ...
          0,
          2 * Math.PI
        ); // 2 * Math.PI represents a full circle (360 degrees)
        ctx.fill();
      }
    }
