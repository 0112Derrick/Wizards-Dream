...
class UtilFunctions {
    findOverworldMapByName(selectedMap: $MapNames, searchingList: Array<$GameMap>): $GameMap | null {
        let foundMap = null;
    ...
    removeObjectFromArray(list: Array<any>, propertyToMatch: string, identifier: any): Array<any> | null {
        if (list.length == 0) {
            return null;
        }
        let returnArr = [];
    ...
    checkIfObjectMeetsCharacterDataInterface(object: any, testing = false): boolean | { result: boolean, data: string } {
    ...
    _testCheckIfObjectIsCharacter(): string {
        let resultA: boolean, resultB: boolean, resultC: boolean, resultD: boolean = false;
        let objectA = {};
        ...
        if (returnValA.result == false) { testResults += "test checkIfObjectIsCharacter \n blank object: success"; resultA = true; } else { testResults += "test checkIfObjectIsCharacter \n blank Object: failure " + returnValA.data; }
        if (returnValB.result == true) { testResults += "test checkIfObjectIsCharacter \n Filled in object : success"; resultB = true; } else { testResults += "test checkIfObjectIsCharacter \n Filled in Object: failure " + returnValB.data; };
        if (returnValC.result == false) { testResults += "test checkIfObjectIsCharacter \n As an array : success"; resultC = true; } else { testResults += "test checkIfObjectIsCharacter \n Character data as an Array: failure " + returnValC.data; };
        if (returnValD.result == false) { testResults += "test checkIfObjectIsCharacter \n blank array: success"; resultD = true; } else { testResults += "test checkIfObjectIsCharacter \n blank array: failure " + returnValD.data; };
